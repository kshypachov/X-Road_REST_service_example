...
1. Відредагуйте скрипт `deploy.sh`, замінивши у секції `[database]` значення змінних `name`, `username`, `password`, база даних з цими параметрами буде встановлена скриптом. Завантажте тільки скрипт `deploy.sh`. Скрипт автоматично встановить СУБД MariaDB, створить користувача, БД (користувача БД, та структуру БД)
